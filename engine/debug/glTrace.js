export function installGLTraceFile(gl, { logEvery = 1, name = "gltrace" } = {}) {
  ...
  const now = () => +(performance.now() - t0).toFixed(2);
  ...
  const trace = {
    ...
    mark(label, extra = null) {
      ...
      const rec = {
        ...
        program: gl.getParameter(gl.CURRENT_PROGRAM) ? "[WebGLProgram]" : null,
        fbo: gl.getParameter(gl.FRAMEBUFFER_BINDING) ? "[WebGLFramebuffer]" : null,
        err: gl.getError(),
        extra,
      };

      this.marks.push(rec);
      rows.push({ op: "mark", ...rec });
    },
    ...
    nextFrame() {
      ...
      rows.push({ op: "frame", t: now(), frame: this.frame, scope: this.scope, marks: this.marks.length });
      this.marks.length = 0;
    },
  ...
  // оборачиваем ключевые вызовы — но ПИШЕМ В rows, не в console
  const wrap = (name) => {
    const orig = gl[name].bind(gl);
    gl[name] = (...args) => {
      const doLog = trace.enabled && trace.frame % trace.logEvery === 0;
      if (doLog) rows.push({ op: `gl.${name}`, t: now(), frame: trace.frame, scope: trace.scope, args: safeArgs(args) });

      const res = orig(...args);

      if (doLog) {
        const err = gl.getError();
        if (err !== gl.NO_ERROR) rows.push({ op: "gl.error", t: now(), frame: trace.frame, scope: trace.scope, after: name, err });
      }
      return res;
    };
  };
  ...
  [
    ...
    "drawArrays",
    "drawElements",
  ].filter(n => typeof gl[n] === "function").forEach(wrap);
