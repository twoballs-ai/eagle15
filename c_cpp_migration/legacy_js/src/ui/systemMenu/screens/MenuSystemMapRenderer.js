// ui/systemMenu/screens/MenuSystemMapRenderer.js
// Рендерит карту системы в СВОЙ WebGL2 canvas (не в game.canvas).
// Поэтому DOM-меню может быть непрозрачным, карта всё равно сверху (внутри меню).
...
// минимальные матрицы (без зависимостей)
...
export class MenuSystemMapRenderer {
  constructor(canvas) {
    this.canvas = canvas;
  ...
  setSize(cssW, cssH, dpr = 1) {
    ...
    // чтобы DOM-слой совпадал по размерам
  ...
  draw(game, scene) {
    ...
    // VP
    const aspect = w / h;
    const halfH = this._cam.orthoSize ?? 1000;
    ...
    // орбита “границы”
    this._drawOrbit(ctx.boundsRadius ?? 1200, 220, [0.95, 0.25, 0.25, 0.45], y);

    // планеты
    for (const p of ctx.system.planets || []) {
      this._drawOrbit(p.orbitRadius, 160, [0.3, 0.3, 0.35, 0.25], y);

      const a = (ctx.time ?? 0) * (p.speed ?? 0) + (p.phase ?? 0);
  ...
  _drawOrbit(radius, segments = 160, colorRGBA = [0.3, 0.3, 0.35, 0.25], y = 0.12) {
    ...
    for (let i = 0; i < segments; i++) {
      ...
      const o = i * 3;
      arr[o + 0] = Math.cos(a) * radius;
      arr[o + 1] = y;
      arr[o + 2] = Math.sin(a) * radius;
    }
