...
import { saveGeneratedPresetToFolder } from "../../../data/system/devSystemPrompt.js";
...
const PLANET_MODEL_OPTIONS = [
  ...ASSETS.planetModels.map((m) => [m, m.split("/").pop()]),
];
...
export class ObjectGeneratorScreen {
  ...
  _buildControls(host) {
    ...
    this.controls.planetPreset = mkSelect(Object.entries(PLANET_PRESETS).map(([k, v]) => [k, v.label]));

    mkLabel("Модель планеты (GLB)");
    this.controls.planetModel = mkSelect(PLANET_MODEL_OPTIONS);

    mkLabel("Пресет звезды");
    this.controls.starPreset = mkSelect(Object.entries(STAR_PRESETS).map(([k, v]) => [k, v.label]));
    ...
    for (const el of Object.values(this.controls)) el.addEventListener?.("input", () => this._drawPreview());
  ...
  async _handleImportModel() {
    ...
    try {
      ...
      this.statusEl.textContent = `Ошибка импорта GLB: ${e?.message ?? e}`;
    }
  }

  _syncPresetDefaults() {
    const pp = PLANET_PRESETS[this.controls.planetPreset.value] ?? PLANET_PRESETS.ocean;
    this.controls.oceans.value = String(Math.round(pp.oceans * 100));
    this.controls.greenery.value = String(Math.round(pp.greenery * 100));
    this.controls.rocks.value = String(Math.round(pp.rocks * 100));
    ...
    const sp = STAR_PRESETS[this.controls.starPreset.value] ?? STAR_PRESETS.yellow;
    this.controls.r.value = String(Math.round(sp.color[0] * 255));
    this.controls.g.value = String(Math.round(sp.color[1] * 255));
    this.controls.b.value = String(Math.round(sp.color[2] * 255));
    this.controls.size.value = String(Math.round(sp.radiusMul * 100));
  ...
  _buildPreset(ctx) {
    ...
    const starColor = [Number(this.controls.r.value) / 255, Number(this.controls.g.value) / 255, Number(this.controls.b.value) / 255];
    const sizeMul = Number(this.controls.size.value) / 100;

    const preserveStar = current?.star ? {
      radiusMul: 1,
      color: [...(current.star.color ?? [1, 0.9, 0.6])],
      visual: { ...(current.star.visual ?? {}) },
      modelUrl: current.star.modelUrl ?? "assets_folder/models/Sun.glb",
    } : null;

    const preservePlanets = (current?.planets ?? []).map((p, i) => ({
      id: p.id ?? `keep-${i}`,
    ...
    if (type === "star") {
      const star = {
        ...
        visual: {
          ...
          emissive: Number(this.controls.emissive.value) / 100,
          ambient: 1.0,
        },
      };
      return {
    ...
    const generatedPlanet = {
      ...
      orbitRadius: preservePlanets[0]?.orbitRadius ?? 620,
      sizeMul,
      visual: {
        type: this.controls.planetPreset.value,
        oceans: Number(this.controls.oceans.value) > 10,
        oceanIntensity: Number(this.controls.oceans.value) / 100,
        greenery: Number(this.controls.greenery.value) / 100,
        rocks: Number(this.controls.rocks.value) / 100,
        satellitesCount: Number(this.controls.satellites.value),
        ...
        ambient: 0.78 + Number(this.controls.greenery.value) / 500,
        emissive: Number(this.controls.rocks.value) / 1000,
      },
    };
  ...
  async _savePreset() {
    const scenes = this.services.get("scenes");
    const ctx = scenes?.current?.ctx;
    const sid = ctx?.systemId ?? this.services.get("state")?.currentSystemId ?? "system";
    const preset = this.lastPreset ?? this._buildPreset(ctx);
    ...
    try {
      ...
      this.statusEl.textContent = `Ошибка сохранения: ${e?.message ?? e}`;
    }
  }
}
