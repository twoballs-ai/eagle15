...
export class MarketScreen {
  ...
  _changeCredits(delta) {
    ...
    const next = Math.max(0, Math.floor((state.credits ?? 0) + delta));
    if (delta < 0 && next === 0 && (state.credits ?? 0) + delta < 0) return false;
    state.credits = next;
    return true;
  }
  ...
  _buy(itemId, n = 1) {
    ...
    if ((state.credits ?? 0) < total) return;
  ...
  _sell(itemId, n = 1) {
    ...
    this._changeCredits(Math.floor(getPrice(itemId) * n * 0.6));
    this.refresh();
  }
  ...
  refresh() {
    ...
    this._creditsEl.textContent = `Кредиты: ${state.credits ?? 0}`;
    this._listEl.innerHTML = "";

    const ids = new Set();
    for (const [id] of inv.entriesSorted()) ids.add(id);
    ["oxygen", "iron_ore", "copper_ore", "silicon_dust", "polymer_slurry", "iron_ingot", "copper_wire", "ship_thruster_module"].forEach((id) => ids.add(id));
    ...
    for (const id of [...ids].sort()) {
      ...
      b1.disabled = (state.credits ?? 0) < price;
