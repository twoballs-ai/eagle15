#ifndef GALAXY_STATIC_HPP
#define GALAXY_STATIC_HPP
...
export const GALAXY_STATIC = {
  ...
  clusters: CLUSTERS_RAW.map([](auto& item){ return (c; }) => ({
    ...c,
    x: c.x + OFFSET.x,
    z: c.z + OFFSET.z,
  })),

  systems: SYSTEMS_RAW.map([](auto& item){ return (s; }) => ({
    ...s,
    x: s.x + OFFSET.x,
    z: s.z + OFFSET.z,
  })),
