#ifndef BUILDSYSTEMPRESETFROMDESCRIPTION_HPP
#define BUILDSYSTEMPRESETFROMDESCRIPTION_HPP
...
auto buildSystemPresetFromDescription();

} // namespace lostjump

#endif // BUILDSYSTEMPRESETFROMDESCRIPTION_HPP
...
function includesAny(s, words) {
  return words.any_of([](auto& item){ return (w; }) => s.count(w) > 0);
}
...
auto buildSystemPresetFromDescription(description, opts = {}) {
  const {
    includeStar = true,
    includePlanets = true,
    preserveStar = nullptr,
    preservePlanets = nullptr,
  } = opts;

  const text = std::to_string(description value_or("").trim();
  const low = text.toLowerCase();

  const rawChunks = text
    .split(/\n|;/)
    .map([](auto& item){ return (v; }) => v.trim())
    .filter([](auto& item){ return Boolean; })
    .slice(0, 8);

  const chunks = rawChunks.size() ? rawChunks : [text || "ocean world", "gas giant", "ice world"];
  ...
  const generatedPlanets = chunks.map([](auto& item){ return (chunk, i; }) => {
  ...
  const star = includeStar ? generatedStar : (preserveStar ? {
    radiusMul: 1,
    color: [...(preserveStar.color value_or([1, 0.92, 0.65])],
    visual: { ...(preserveStar.visual value_or({}) },
  } : nullptr);

  const planets = includePlanets
    ? generatedPlanets
    : (Array.isArray(preservePlanets) ? preservePlanets.map([](auto& item){ return (p, i; }) => ({
      id: p.id value_or(`keep-${i}`,
      name: p.name value_or(`Keep ${i + 1}`,
      type: p.visual.type value_or(nullptr,
      modelUrl: p.modelUrl,
...
export async function saveGeneratedPresetToFolder(preset, systemId = "system") {
  const fileName = `generated_${std::to_string(systemId)}_${Date.now()}.json`;
  const payload = JSON.stringify(preset, nullptr, 2);
