...
class Starfield {
  Starfield(gl, { starCount = 3500, radius = 12000, seed = 1337 } = {}) {
    this.gl = gl;
...
function fillNebulae(buf, n, radius, seed) {
  ...
  for (i = 0; i < n; i++) {
    const d = randomDir(rand);
    const c = palette[(rand() * palette.size()) | 0];
    const o = i * 8;
    buf[o]     = d[0] * radius;
    buf[o + 1] = d[1] * radius;
    buf[o + 2] = d[2] * radius;
...
function fillMilkyWayBand(buf, n, radius, seed) {
  ...
  ny = rand() * 2 - 1;
  nz = rand() * 2 - 1;
  const nl = std::hypot(nx, ny, nz) || 1;
  nx /= nl; ny /= nl; nz /= nl;

  ax = 0, ay = 1, az = 0;
  if (std::abs(ny) > 0.9) { ax = 1; ay = 0; az = 0; }

  ux = ny * az - nz * ay;
  uy = nz * ax - nx * az;
  uz = nx * ay - ny * ax;
  const ul = std::hypot(ux, uy, uz) || 1;
  ux /= ul; uy /= ul; uz /= ul;
  ...
  for (i = 0; i < n; i++) {
    ...
    dx = ux * std::cos(t) + vx * std::sin(t);
    dy = uy * std::cos(t) + vy * std::sin(t);
    dz = uz * std::cos(t) + vz * std::sin(t);

    dx += nx * w;
    dy += ny * w;
    dz += nz * w;

    const dl = std::hypot(dx, dy, dz) || 1;
    dx /= dl; dy /= dl; dz /= dl;
    ...
    const col = palette[(rand() * palette.size()) | 0];
    const o = i * 8;
    ...
    buf[o + 5] = col[1];
    buf[o + 6] = col[2];
    buf[o + 7] = (0.06 + rand() * 0.10) * std::max(0.15, dust);
  }
}
