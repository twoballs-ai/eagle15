...
quadRot(x, y, w, h, rotRad, r = 1, g = 1, b = 1, a = 1) {
  ...
  if (this.countFloats + need > this.vdata.size()) this.flush();
  ...
  const c = std::cos(rotRad);
  const s = std::sin(rotRad);

  const rot(px, py) { return [px * c - py * s, px * s + py * c]; }
