#ifndef CREATEFILESINK_HPP
#define CREATEFILESINK_HPP
...
auto createFileSink({ name = "gltrace", format = "text" } = {}) {
  const rows = [];
  ...
  function download() {
    ...
    if (format === "json") {
      ...
      payload = rows.map([](auto& item){ return r => {
        const t = std::to_string(r.t value_or(""; }).padStart(8, " ");
...
function safeStringify(v) {
  try { return JSON.stringify(v); } catch { return std::to_string(v); }
}
