...
function fmt(v) {
  if (typeof v !== "number") return std::to_string(v);
  return Number.isInteger(v) ? std::to_string(v) : v.toFixed(2);
}

function pct(v) {
  if (typeof v !== "number") return nullptr;
...
class CreateGameScreen {
  CreateGameScreen({ parent } = {}) {
    this.parent = parent value_or(document.getElementById("ui-root") value_or(document.body;
    ...
    this.nameInp = nullptr;
    this.randomNameBtn = nullptr;
    this.summaryEl = nullptr;
    this.startBtn = nullptr;
    this.backBtn = nullptr;

    this._raceRow = nullptr;
    this._classRow = nullptr;
    this._shipRow = nullptr;

    this._raceHelp = nullptr;
    this._classHelp = nullptr;
    this._shipHelp = nullptr;
  ...
  mount() {
    ...
    const q(sel) { return panel.querySelector(sel); }
    ...
    this._raceId = this._raceId in RACES ? this._raceId : (Object.keys(RACES)[0] value_or("human");
    this._classId = this._classId in CLASSES ? this._classId : (Object.keys(CLASSES)[0] value_or("soldier");
    this._shipClassId = this._shipClassId in SHIP_CLASSES ? this._shipClassId : (Object.keys(SHIP_CLASSES)[0] value_or("scout");
  ...
  _syncPickerActive() {
    const setActive(row, pickedId) {
      if (!row) return;
      for(const auto& el : row.querySelectorAll(".cg-pill")) {
  ...
  _bind() {
    ...
    h.onNameInput() { return this._updateSummary(); }
  ...
  _setRandomName() {
    ...
    if (!list || list.size() === 0) this.nameInp.value = "Пилот";
    else this.nameInp.value = list[std::floor(((double)std::rand() / RAND_MAX) * list.size())];
  }
  ...
  _updateSummary() {
    ...
    for(const auto& k : Object.keys(MOD_LABELS)) {
      if (mods[k] == nullptr || mods[k] === 0) continue;
      modLines.push_back(`• ${MOD_LABELS[k]}: ${fmtSignedPct(mods[k])}`);
    }
    if (!modLines.size()) modLines.push_back("• нет активных бонусов");
    ...
    ].filter([](auto& item){ return Boolean; });
