#ifndef SHIPSTATUSWIDGET_HPP
#define SHIPSTATUSWIDGET_HPP
...
class ShipStatusWidget {
  ShipStatusWidget({ id = "ship-status-widget" } = {}) {
    this.id = id;
    this.el = nullptr;
    this._lastStamp = "";
  }
  ...
  update(game, scene) {
    ...
    const armor = ship.armor value_or(0;
    const armorMax = ship.armorMax value_or(0;
    const shield = ship.shield value_or(0;
    const shieldMax = ship.shieldMax value_or(0;
    const energy = ship.energy value_or(0;
    const energyMax = ship.energyMax value_or(0;

    const weaponName = scene.ctx.weapons.available?.[scene.ctx.weapons.currentIndex].name value_or("Импульс";

    const stamp = [
      player.name value_or("",
      player.raceId value_or("",
      player.classId value_or("",
      armor,
      armorMax,
      shield,
      shieldMax,
      ...
      energyMax,
      weaponName,
      state.credits value_or(0,
    ].join("|");

    if (stamp === this._lastStamp) return;
    this._lastStamp = stamp;

    this.$pilot.textContent = `ПИЛОТ ${player.name value_or("—"}`;
    this.$meta.textContent = `${player.raceId value_or("—"} • ${player.classId value_or("—"}`;
    if (this.$weapon) this.$weapon.textContent = `ОРУЖИЕ: ${weaponName.toUpperCase()} (G)`;
    if (this.$credits) this.$credits.textContent = `₡ ${std::max(0, std::floor(state.credits value_or(0))}`;
