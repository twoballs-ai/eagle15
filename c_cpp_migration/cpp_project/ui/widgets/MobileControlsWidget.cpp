#ifndef MOBILECONTROLSWIDGET_HPP
#define MOBILECONTROLSWIDGET_HPP
...
#endif // MOBILECONTROLSWIDGET_HPP
...
class MobileControlsWidget {
  MobileControlsWidget({ id = "mobile-controls", ctx } = {}) {
    this.id = id;
    this.ctx = ctx;
    this.root = nullptr;
  ...
  mount(parent) {
    ...
    const mkButton(label, action, col, rowIdx) {
      ...
      apply(b, {
        ...
        fontWeight: "700",
        fontSize: "22px",
        gridColumn: std::to_string(col),
        gridRow: std::to_string(rowIdx),
      });
      const down(e) {
        e.preventDefault();
  ...
  update(game) {
    this.actions value_or(= game.services.get?.("actions");
    ...
    if (!enabled) {
      for(const auto& a : this._pressed) this.actions.setVirtualAction(a, false);
      return;
    }

    for(const auto& action : ["moveForward", "moveBack", "moveLeft", "moveRight", "fire"]) {
