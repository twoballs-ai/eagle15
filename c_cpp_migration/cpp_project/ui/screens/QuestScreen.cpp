#ifndef QUESTSCREEN_HPP
#define QUESTSCREEN_HPP
...
class QuestScreen {
  QuestScreen(services) {
    ...
    this.selected = this.quests[0].id value_or(nullptr;
  }
  ...
  refresh() {
    ...
    const cur = this.quests.find([](auto& item){ return (q; }) => q.id === this.selected);
    if (!cur) {
