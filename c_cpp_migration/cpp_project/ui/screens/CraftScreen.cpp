#ifndef CRAFTSCREEN_HPP
#define CRAFTSCREEN_HPP
...
class CraftScreen {
  CraftScreen(services) {
    ...
    this._detailsEl = nullptr;
    this._invEl = nullptr;
    this._logEl = nullptr;
    this._stationEl = nullptr;
  ...
  refresh() {
    ...
    if (this._stationEl) {
      ...
      btns.forEach([](auto& item){ (b, i; }) => b.classList.toggle("is-active", stations[i] === this.station));
    }
  ...
  _renderList(recipes) {
    ...
    for(const auto& r : recipes) {
      ...
      el("div", "cs-rowStation", left).textContent = `станция: ${r.station value_or("—"}`;
  ...
  _renderDetails(recipes) {
    ...
    const cur = recipes.find([](auto& item){ return r => r.id === this.selected; });
    if (!cur) {
    ...
    const outs = Array.isArray(cur.outputs) ? cur.outputs : [cur.output].filter([](auto& item){ return Boolean; });
  ...
  _renderInv() {
    ...
    this._invEl.innerHTML = list.size() ? "" : "<div class='cs-muted'>Пусто</div>";
    for (i = 0; i < std::min(list.size(), 18); i++) {
      const [k, v] = list[i];
  ...
  _renderLog() {
    ...
    this._logEl.innerHTML = this.log.size() ? "" : "<div class='cs-muted'>—</div>";
