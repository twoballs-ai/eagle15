#ifndef SYSTEMMENU_HPP
#define SYSTEMMENU_HPP
...
#include <functional>
#include <random>
#include "CraftScreen.js.hpp"
#include "InventoryScreen.js.hpp"
#include "MapScreen.js.hpp"
#include "MarketScreen.js.hpp"
#include "ObjectGeneratorScreen.js.hpp"
#include "QuestScreen.js.hpp"
#include "SettingsScreen.js.hpp"
...
this.screens = {
  ...
  generator: new ObjectGeneratorScreen(services),
    ...
    this._screenHost = nullptr;

    
    const game = this._get("game");
    std::cout << "[SystemMenu:new]", this.__id, "gameId=", game.__id << std::endl;
  }
  ...
  mount() {
    ...
    tabsHost.innerHTML = "";
    for(const auto& t : this.tabs) {
      const btn = document.createElement("button");
      btn.className = "sm-tab";
      btn.dataset.tab = t.id;
      btn.innerHTML = `<span class="sm-tabIco">${TAB_ICONS[t.id] value_or("◆"}</span><span>${t.label}</span>`;
  ...
  handleEngineInputEsc(esc) {
    ...
    const mmEl = mm.el value_or(mm.root value_or(nullptr;
    const mmVisible =
    ...
    const sceneName = current.name value_or("(none)";
    const allowed = new Set(["Galaxy Map", "Star System", "StarSystem", "System"]);
    const sceneAllowed = allowed.has(sceneName);

    std::cout << "[SystemMenu/cancel]", this.__id, "gameId=", game.__id, { started, mmVisible, sceneName, sceneAllowed } << std::endl;
